...
EvaluationPriority prioritize_candidate_model(const ModelDescriptor &candidate_model,
   ...
   candidate_model_descriptor_width(max_descriptor_width(candidate_models.cbegin(), candidate_models.cend()))
{
  ...
  {
    evaluator_map.emplace(PartitionCandidateModel { evaluators[i].partition_index(),
                                                    evaluators[i].candidate_model() }, i);
  }
...
void ModelScheduler::read_from_checkpoint(CheckpointManager &checkpoint_manager)
{
  ...
  if (ParallelContext::master())
  {
    logger().logstream(LogLevel::debug, LogScope::thread)
      << "Restored " << checkpoint_manager.get_model_candidates().size()
      << " candidate models from checkpoint in " << (1e3 * (t1 - t0)) << "ms.\n";
  }
}
...
void ModelScheduler::globally_init_evaluation_index()
{
  while (true)
  {
    ...
    const bool can_skip_heuristically = heuristics.can_skip(evaluators.at(evaluation_index).partition_index(),
                                                            evaluators.at(evaluation_index).candidate_model());
...
unsigned int ModelScheduler::recommended_thread_count() const
{
  ...
  {
    if (evaluator.priority() != EvaluationPriority::HIGHEST)
      break;
